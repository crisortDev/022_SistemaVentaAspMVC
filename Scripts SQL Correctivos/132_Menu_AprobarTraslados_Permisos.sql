-- ════════════════════════════════════════════════════════════════════════════════
-- SCRIPT 132: Submenú "Aprobar Traslados" + permisos
-- Base de datos: [DBVENTAS_WEB]
-- Fecha: 2026-06-11
--
-- Crea el submenú "Aprobar Traslados" (Controlador=Inventario, Vista=AprobarTraslados)
-- y le asigna permiso a: Encargado(6), Supervisor(11), Administrador(1), SuperAdmin(14).
-- El que carga el traslado (Repositor/Cajero) NO aprueba (segregación).
--
...
-- Crear submenú "Aprobar Traslados" si no existe
DECLARE @IdSm INT;
SELECT @IdSm = IdSubMenu FROM dbo.SUBMENU
WHERE Controlador = 'Inventario' AND Vista = 'AprobarTraslados';

IF @IdSm IS NULL
BEGIN
    INSERT INTO dbo.SUBMENU (IdMenu, Nombre, Controlador, Vista, Icono, Orden, Activo)
    VALUES (@IdMenu, 'Aprobar Traslados', 'Inventario', 'AprobarTraslados', 'fas fa-check-double', 51, 1);
    SET @IdSm = SCOPE_IDENTITY();
    PRINT 'OK: submenú "Aprobar Traslados" creado (IdSubMenu=' + CAST(@IdSm AS VARCHAR) + ').';
END
ELSE
BEGIN
    UPDATE dbo.SUBMENU SET Activo = 1, Nombre = 'Aprobar Traslados' WHERE IdSubMenu = @IdSm;
    PRINT 'INFO: submenú "Aprobar Traslados" ya existía (IdSubMenu=' + CAST(@IdSm AS VARCHAR) + '), reactivado.';
END
...
PRINT 'OK: permisos de "Aprobar Traslados" asignados (Encargado, Supervisor, Admin, SuperAdmin).';
GO
...
WHERE s.Controlador = 'Inventario' AND s.Vista = 'AprobarTraslados'
ORDER BY r.IdRol;
GO
