-- ════════════════════════════════════════════════════════════════════════════════
-- SCRIPT 21: Agregar IdOrdenPago a usp_ObtenerListaCompraRevision
-- Base de datos: [DBVENTAS_WEB]
-- Fecha: 2026-05-07
--
-- Permite que la vista Revisión de Compras detecte si una compra
-- ya tiene una Orden de Pago generada, para mostrar/ocultar el botón "Generar OP".
--
-- Cambio: LEFT JOIN con ORDEN_PAGO → retorna IdOrdenPago (0 si no existe aún).
-- ════════════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    SELECT
        ...
        CASE
            ...
            ELSE 0
        END                                                  AS NecesitaNC,
        -- Orden de Pago: 0 si aún no se generó, > 0 si ya existe
        ISNULL(op.IdOrdenPago, 0)                            AS IdOrdenPago
    FROM dbo.COMPRA c
    ...
    LEFT  JOIN dbo.ORDEN_PAGO        op  ON op.IdCompra     = c.IdCompra
    WHERE c.Activo = 1
...
PRINT 'OK: usp_ObtenerListaCompraRevision actualizado — incluye IdOrdenPago'
PRINT 'El botón Generar OP aparecerá en Revisión solo para compras Confirmadas sin OP.'
